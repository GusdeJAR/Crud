  ...
  const InstitutosTable = () => {
    ...
    async function fetchData() {
      try {
        ...
        //const Institutoos = await getInstitutos();
    ...
    const handleDelete = async () => {
      const res = await swal({ title: "¿Estás seguro?", 
        text: `El producto con el ID ${InstitutoSel.IdInstitutoOK} será eliminado, ¿Desea continuar?`, 
        icon: "warning", 
        buttons: true, 
        dangerMode: true, });
      if (res) {
                ...
                <Stack direction="row" sx={{ m: 1 }}>
                  <Box>
                    ...
                    <Tooltip title="Editar">
                      <IconButton onClick={() => setUpdateInstitutoShowModal(true)}>
                        <EditIcon />
                      </IconButton>
                    </Tooltip>
